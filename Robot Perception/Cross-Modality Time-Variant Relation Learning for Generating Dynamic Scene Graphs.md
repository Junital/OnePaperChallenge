...
如下图所示，本文的跨模态特征会引导时间差异，获取到$e_{r_t} - e_{r_t-1}$和$e_{s_t} - e_{s_t-1}$。在训练过程中，尽可能让两者之间的差距减小。

![Fig2](./Fig/Cross-Modality%20Feature%20Guidance.png)

## 检验和测试

一共有9848个视频片段、23万个帧。最后，$\text{TR}^2$的表现超过了目前最好的方法。

## 个人感想

思路很不错，特别是将“拆一拆”的思想非常深刻地融入到了里面。比如分别从时间、空间上进行研究；从直接和间接方向进行研究。
